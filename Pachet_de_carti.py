...
class Deck:
    ...
    def afiseaza_pachet(self):
        ...
        for carte in self.deck:
            pachet.append(carte)
            # i += 1
        return pachet #str(pachet) # f'Cartile sunt: \n {pachet}'
        

    def amesteca_pachet(self):
        'Amesteca cartile'
        rng.shuffle(self.deck)
        return f'Cartile amestecate cu succes \n'
        ...
        if type(carte) is str:  # eliminare dupa nume
            ...
            for c in self.deck:
                if c.valoare == valoare_carte and c.simbol == simbol_carte:
                    self.deck.remove(c)
                    return f'[{carte}] eliminata \n'
                    
            return f'[{carte}] tip invalid de carte \n'

        elif type(carte) is int:  # eliminare dupa index
            if carte in range(len(self.deck)):
                self.deck.pop(carte)
                return f'[{self.deck[carte]}] eliminata \n'
            else:
    ...
    def adauga_carte(self, nume_carte):
        ...
        if len(self.deck) < 51:
            ...
            if valoare_carte in Carte.valoare and simbol_carte in Carte.simbol:            
                for c in self.deck:
                    if c.valoare == valoare_carte and c.simbol == simbol_carte:
                        return f'[{nume_carte}] se afla deja in pachet pe pozitia {self.deck.index(c) + 1} \n'      
                self.deck.append(Carte(valoare_carte, simbol_carte))
                return f'[{nume_carte}] adaugata cu succes \n'
            else:
                return f'[{nume_carte}] nume invalid de carte \n'
        else:
            return 'Pachetul este plin, nu se pot adauga carti.'

def Curs_7_Ex_1():
    deck = Deck()
    return deck.afiseaza_pachet()
